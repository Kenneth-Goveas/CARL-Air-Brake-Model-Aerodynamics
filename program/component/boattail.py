...
class Boattail:
    ...
    def __init__ (self, **kwargs):
        ...
        btl_pos = kwargs['btl_pos']
        btl_len = kwargs['btl_len']
        btl_for_rad = kwargs['btl_for_rad']
        btl_aft_rad = kwargs['btl_aft_rad']

        prf_div = 10000
        prf_pos = np.linspace(btl_pos, btl_pos + btl_len, prf_div + 1)
        prf_rad = btl_for_rad + (prf_pos - btl_pos) * (btl_aft_rad - btl_for_rad) / btl_len
        prf_slp = np.full(prf_div + 1, (btl_aft_rad - btl_for_rad) / btl_len)
    ...
    def _getPresDragCoef (self, **kwargs):
        ...
        if self._cmp_len + 2*self._cmp_dif_rad < 0:
            geom_corr = 1
        elif self._cmp_len + 6*self._cmp_dif_rad > 0:
            geom_corr = 0
        else:
            geom_corr = (self._cmp_len + 6*self._cmp_dif_rad) / (4*self._cmp_dif_rad)

        if flw_mach < 1:
            pres_drag_coef = -(0.12 + 0.13*flw_mach**2)
        else:
            pres_drag_coef = -0.25/flw_mach
        pres_drag_coef *= angl_corr * geom_corr
