...
class Nose:
    ...
    def __init__ (self, **kwargs):
        ...
        nos_len = kwargs['nos_len']
        nos_rad = kwargs['nos_rad']
        nos_ogv = kwargs['nos_ogv']

        prf_div = 10000
        prf_pos = np.linspace(0, nos_len, prf_div + 1)
        if nos_ogv == 0:
            prf_rad = prf_pos * nos_rad / nos_len
            prf_slp = np.full(prf_div + 1, nos_rad / nos_len)
        else:
            crv = np.sqrt((nos_len**2 + nos_rad**2) * ((2 - nos_ogv)**2 * nos_len**2 + nos_ogv**2 * nos_rad**2)) / (2 * nos_ogv * nos_rad)
            prf_rad = np.sqrt(crv**2 - (nos_len / nos_ogv - prf_pos)**2) - np.sqrt(crv**2 - (nos_len / nos_ogv)**2)
            prf_slp = (nos_len / nos_ogv - prf_pos) / np.sqrt(crv**2 - (nos_len / nos_ogv - prf_pos)**2)
    ...
    def _getPresDragCoef (self, **kwargs):
        ...
        geom_corr = 0.72*(self._cmp_ogv - 0.5)**2 + 0.82
